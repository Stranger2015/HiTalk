...
class Mirror {
    ...
    @SafeVarargs
    public static
    <E extends Enum <E>> void of ( Class <E> image, Class <E>... images ) throws IllegalStateException {
    ...
    @SuppressWarnings("unchecked")
    private static
    <E extends Enum <E>> Class <E> discoverCaller () {
        try {
            return (Class <E>) Class.forName(stream(new Throwable().getStackTrace()).filter(e -> !e.getClassName().equals(Mirror.class.getName())).findFirst().orElseThrow(() -> new IllegalStateException(format("Cannot discover caller's class. It seems that %s calls itself", Mirror.class.getName()))).getClassName());
    ...
    @SafeVarargs
    public static
    <T extends Enum <T>, M extends Enum <M>> void mirrors ( Class <T> mirror, Class <M> image, Class <? extends Enum>... images ) {

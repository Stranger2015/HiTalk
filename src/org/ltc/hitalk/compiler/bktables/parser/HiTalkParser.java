...
/**
 * HiTalkParser is a recursive descent parser for the language HiTalk, that parses its input into first order logic
 ...
 */
public
class HiTalkParser extends HiLogParser <Clause> {
    /* Used for debugging purposes. */
    /* private static final Logger log = Logger.getLogger(HiTalkParser.class.getName()); */

    /* Used for logging to the console. */
    private static final java.util.logging.Logger console = java.util.logging.Logger.getLogger("CONSOLE." + HiTalkParser.class.getName());
...
//        this.interner = interner;
     ...
     */
    public
    Clause clause () throws SourceCodeException {
        // Each new sentence provides a new scope in which to make variables unique.
        variableContext.clear();
    ...
    /**
     * Peeks at the next token to see if it is  and if it is consumes it. If the symbol is consumed then the
    ...
    /**
     * Peeks at the next token to see if it is an  which is equal to ";" and if it is consumes it. If the
     ...
     */
    public
    PrologParser.Directive peekAndConsumeDirective () throws SourceCodeException {
        if (peekAndConsumeTrace()) {
            return PrologParser.Directive.Trace;
        }

        if (peekAndConsumeInfo()) {
            return PrologParser.Directive.Info;
        }

        if (peekAndConsumeUser()) {
            return PrologParser.Directive.User;
        }

...
public
class Vfs2NioFileAttributeView implements BasicFileAttributeView {
    private
    enum Attribute {
        attributes,
        certificates,
    ...
    static
    <V extends FileAttributeView> V get ( Vfs2NioPath path, Class <V> type ) {
        if (type == null) throw new NullPointerException();
        if (type == BasicFileAttributeView.class) return (V) new Vfs2NioFileAttributeView(path, false);
        if (type == Vfs2NioFileAttributeView.class) return (V) new Vfs2NioFileAttributeView(path, true);
        return null;
    }

    static
    Vfs2NioFileAttributeView get ( Vfs2NioPath path, String type ) {
        if (type == null) throw new NullPointerException();
        if (type.equals("basic")) return new Vfs2NioFileAttributeView(path, false);
        if (type.equals("vfs")) return new Vfs2NioFileAttributeView(path, true);
        return null;
    }
    ...
    private
    Vfs2NioFileAttributeView ( Vfs2NioPath path, boolean isZipView ) {
        this.path = path;
        this.vfs = isZipView;
    }
    ...
    @Override
    public
    Vfs2NioFileAttributes readAttributes () throws IOException {
    ...
    void setAttribute ( String attribute, Object value ) throws IOException {
        try {
            if (Attribute.valueOf(attribute) == Attribute.lastModifiedTime) setTimes((FileTime) value, null, null);
            return;
        } catch (IllegalArgumentException x) {
        }

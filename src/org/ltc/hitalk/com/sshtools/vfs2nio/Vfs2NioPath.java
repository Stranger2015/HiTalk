...
import org.ltc.hitalk.org.apache.nio.BasePath;
...
public
class Vfs2NioPath extends BasePath <Vfs2NioPath, Vfs2NioFileSystem, Vfs2NioFileSystemProvider> {
    public
    Vfs2NioPath ( Vfs2NioFileSystem fileSystem, String root, ImmutableList <String> names ) {
        super(fileSystem, root, names);
    }

    public
    Vfs2NioPath ( Vfs2NioFileSystem fileSystem, String root, String... names ) {
        super(fileSystem, root, names);
    }
    ...
    Vfs2NioFileAttributes getAttributes () throws IOException {
        ...
        if (zfas == null) throw new NoSuchFileException(toString());
        return zfas;
    }

    FileStore getFileStore () throws IOException {
        // each ZipFileSystem only has one root (as requested for now)
        if (exists()) return getFileSystem().getFileStore(this);
    ...
    void setAttribute ( String attribute, Object value, LinkOption... options ) throws IOException {
        ...
        if (view == null) throw new UnsupportedOperationException("view <" + view + "> is not supported");

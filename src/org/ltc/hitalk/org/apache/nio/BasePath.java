 ...
 * under the License.
 */
package org.ltc.hitalk.org.apache.nio;
...
class BasePath<T extends BasePath <T, FS, P>, FS extends BaseFileSystem <T, P>, P extends FileSystemProvider>
    ...
    public
    BasePath ( FS fileSystem, String root, String... names ) {
    ...
    T relativize ( Path other ) {
        ...
        List <String> extraNamesInOther = (p2.names.size() <= sharedSubsequenceLength) ? Collections.emptyList() : p2.names.subList(sharedSubsequenceLength, p2.names.size());
    ...
    boolean startsWith ( Path other ) {
        T p1 = asT();
        T p2 = checkPath(other);
        return Objects.equals(p1.getFileSystem(), p2.getFileSystem()) && Objects.equals(p1.root, p2.root) && startsWith(p1.names, p2.names);
    }
    ...
    protected
    T create ( String root, Collection <String> names ) {
        return create(root, new ImmutableList <>(names.toArray(new String[names.size()])));
    }

    protected
    T create ( String root, ImmutableList <String> names ) {
        return fileSystem.create(root, names);
    }

    protected
    T create ( String root, String... names ) {
        return create(root, new ImmutableList <>(names));
    }

    protected
    boolean endsWith ( List <?> list, List <?> other ) {
    ...
    protected
    boolean startsWith ( List <?> list, List <?> other ) {

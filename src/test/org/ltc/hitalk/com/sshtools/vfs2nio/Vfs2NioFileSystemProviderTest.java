...
class Vfs2NioFileSystemProviderTest {
    ...
    void testCreateFolder () throws Exception {
        try (FileSystem rootFs = createRootVFS()) {
            String randpath = System.getProperty("java.io.tmpdir") + "/" + (long) (Math.abs(Math.random() * 100000));
    ...
    @Test
    public
    void testFileReadUri () throws Exception {
        File file = File.createTempFile("vfs", "tmp");
        writeTestFile(file);
    ...
    void testRootList () throws Exception {
        try (FileSystem fs = createRootVFS()) {
            ...
            /* Get names directly */
            List <String> directNames = new ArrayList <>(Arrays.asList(Objects.requireNonNull(rootFile.list())));
    ...
    private
    void compareStreams ( InputStream expected, InputStream actual ) throws IOException {
        int r1, r2;
        while (true) {
            r1 = expected.read();
            r2 = actual.read();
    ...
    private
    FileSystem createRootVFS () throws IOException {
    ...
    private
    void writeTestBytes ( OutputStream out ) throws IOException {
    ...
    private
    void writeTestFile ( File file ) throws IOException {

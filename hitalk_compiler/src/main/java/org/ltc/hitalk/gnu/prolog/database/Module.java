...
public class Module {
     ...
     *
     * @param prologTextLoaderError the partial error to be used if this term throws an error
     * @param term                  the goal to execute at initialization
     */
    public synchronized void addInitialization(PrologTextLoaderError prologTextLoaderError, Term term) {
        initialization.add(new Pair<>(prologTextLoaderError, term));
    }
    ...
    public synchronized void clearInitialization() {
        initialization = Collections.synchronizedList(new ArrayList<>());
    }
    ...
    public synchronized void predicateUpdated(CompoundTermTag tag) {
        ...
        for (PredicateListener listener : new ArrayList<>(predicateListeners)) {

...
import org.ltc.hitalk.gnu.prolog.io.PrologStream;
import org.ltc.hitalk.gnu.prolog.term.*;
import org.ltc.hitalk.gnu.prolog.vm.PrologCode.Result;
import org.ltc.hitalk.gnu.prolog.vm.interpreter.Predicate_call;
import org.ltc.hitalk.gnu.prolog.vm.interpreter.Tracer;

import java.util.HashMap;
import java.util.Map;

import static org.ltc.hitalk.gnu.prolog.vm.PrologCode.Result.FAIL;
import static org.ltc.hitalk.gnu.prolog.vm.PrologCode.Result.SUCCESS_LAST;
...
public final class Interpreter implements HasEnvironment {
     ...
     * @param t1
     * @param t2
     * @return {@link Result#SUCCESS_LAST} or {@link Result#FAIL}
     * @throws PrologException
     */
    public Result simpleUnify(Term t1, Term t2) throws PrologException {
        Result rc = SUCCESS_LAST;
        if (t1 != t2) {
     ...
     * @throws PrologException
     */
    public Result unify(Term t1, Term t2) throws PrologException {
        int undoPos = getUndoPosition();
        Result rc = simpleUnify(t1, t2);
        if (rc == FAIL) {
            undo(undoPos);
        }
        return rc;
     ...
     * {@linkorg.ltc.hitalk.gnu.prolog.vm.builtins.io.Predicate_ensure_loaded}
     *
     ...
     */
    private final Map<Goal, ReturnPoint> returnPoints = new HashMap<Goal, ReturnPoint>();
    ...
    public Goal prepareGoal(Term term) {
        ReturnPoint rp = null;
        if (currentGoal != null) {// Take a copy of the current state into a Return point
            rp = new ReturnPoint();
            rp.rContext = context;
        ...
        context.clear();
        if (rp != null) {// save the return point so that we can jump back later
     ...
     * successful this was.
     *
     * @param goal the goal created using {@link #prepareGoal(Term)} which is to be
     *             run.
     * @return {@link Result#SUCCESS SUCCESS}, {@link Result#SUCCESS_LAST}
     * , {@link Result#FAIL} (or {@link Result#HALT})
     * @throws PrologException
     */
    public Result execute(Goal goal) throws PrologException {
        Result result;
        haltExitCode = null;
        try {
            try {
                ...
                try {
                    Result rc = Predicate_call.staticExecute(this, !goal.firstTime, goal.getGoal());
                    switch (rc) {
                        case SUCCESS_LAST:
                        case FAIL:
                            goal.stopped = true;
     ...
     * {@link #execute(Goal)}.
     *
     * @param goalTerm the term to be executed
     * @return {@link PrologCode.Result#SUCCESS}, {@link PrologCode.Result#SUCCESS_LAST} or
     * {@link PrologCode.Result#FAIL}
     * @throws PrologException
     */
    public Result runOnce(Term goalTerm) throws PrologException {
    ...
    /**
     * Only call this method if you have had {@link PrologCode.Result#HALT} returned by

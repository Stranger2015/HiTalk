 ...
 */
package org.ltc.hitalk.gnu.prolog.io.parser.gen;

import com.thesett.aima.logic.fol.isoprologparser.TokenSource;
import org.ltc.hitalk.gnu.prolog.io.Operator;
import org.ltc.hitalk.gnu.prolog.io.ReadOptions;
import org.ltc.hitalk.gnu.prolog.io.parser.NameToken;
import org.ltc.hitalk.gnu.prolog.io.parser.ReaderCharStream;
...
import static org.ltc.hitalk.gnu.prolog.io.parser.TermParserUtils.EOF_TOKEN;
import static org.ltc.hitalk.gnu.prolog.io.parser.TermParserUtils.convertQuotedString;
import static org.ltc.hitalk.gnu.prolog.vm.TermConstants.listTag;

public class TermParser implements TermParserConstants {
    CharStream stream;
    protected Environment environment;
    protected final Deque<TokenSource> tokenSourceStack= new ArrayDeque<>();
    ...
    boolean testOp(ReadOptions options) {
        Token tk = getToken(1);
    ...
    Term exp(ReadOptions options) throws ParseException {
        Term t;
        if (testOp(options) && isExpSeparator(2)) {
            t = name();
        } else {
    ...
    final public CompoundTermTag op2(ReadOptions options, int priority) throws ParseException {
        ...
        if (jj_2_2(2147483647) && priority >= 1000) {
            ...
            if ((jj_ntk == -1 ? jj_ntk() : jj_ntk) == NAME_TOKEN) {
                jj_consume_token(NAME_TOKEN);
                NameToken tk = (NameToken) token;
                return tk.xfOp.tag;
            } else {
    ...
    final public CompoundTerm compound(ReadOptions options) throws ParseException {
        ...
        IntStream.range(0, n).forEach(i -> rc.args[i] = args.get(i));
        {
            if (true) {
                return rc;
            }
    ...
    final public Term char_code_list() throws ParseException {
        ...
        String val = convertQuotedString(token.image, '\"');
        // Get the Atom form
        ...
        if (pos == jj_endpos + 1) {
            ...
            for (Enumeration enum_ = jj_expentries.elements(); enum_.hasMoreElements(); ) {
                ...
                if (oldentry.length == jj_expentry.length) {
                    exists = !IntStream.range(0, jj_expentry.length).anyMatch(i -> oldentry[i] != jj_expentry[i]);
                    if (exists) {
                        break;
                    }
                }
    ...
    final public ParseException generateParseException() {
        ...
        IntStream.range(0, 92).filter(i -> la1tokens[i]).forEach(i -> {
    ...
    public void setTokenSource(PlLexer tokenSource) {
        this.tokenSourceStack.push(tokenSource);
    }

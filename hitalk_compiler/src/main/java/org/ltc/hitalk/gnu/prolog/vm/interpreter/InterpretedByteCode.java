...
import org.ltc.hitalk.gnu.prolog.database.Pair;
import org.ltc.hitalk.gnu.prolog.io.TermWriter;
import org.ltc.hitalk.gnu.prolog.term.*;
import org.ltc.hitalk.gnu.prolog.vm.*;
import org.ltc.hitalk.gnu.prolog.vm.interpreter.Tracer.TraceLevel;
import org.ltc.hitalk.gnu.prolog.vm.interpreter.instruction.*;
...
public class InterpretedByteCode implements PrologCode, PrologCodeListener {
    ...
    protected InterpretedByteCode(CompoundTermTag codeTag,
        ...
        Map<CompoundTermTag, Integer> tag2idx = new HashMap<>();
        Map<AtomicTerm, Integer> constant2idx = new HashMap<>();
    ...
    protected void pass1(Instruction[] isrc, int[] ipos, Map<CompoundTermTag, Integer> tag2idx,
        ...
        Set<CompoundTermTag> callTags = new HashSet<>();
        Set<CompoundTermTag> createCompoundTermTags = new HashSet<>();
        Set<AtomicTerm> constantSet = new HashSet<>();
    ...
    protected void pass2(Instruction[] isrc,
        ...
        for (i = 0; i < n; i++) {
            if (isrc[i] instanceof IAllocate) {
                ...
                IPushConstant ii = (IPushConstant) isrc[i];
                int idx = constant2idx.get(ii.term);
            ...
            try {
                try {
                    ...
                    switch (instr) {
                        ...
                        case IJUMP: {
                            currentPosition = ((instructions[currentPosition + 1] & 255) << 8)
                        ...
                        case IRETRY_ME_ELSE: {
                            cur_bi.codePosition = ((instructions[currentPosition + 1] & 255) << 8)
                                                 + (instructions[currentPosition + 2] & 255);
                            interpreter.pushBacktrackInfo(cur_bi);
                            currentPosition += 3;
                            backtrackMode = false;
                        ...
                        case IUNIFY: {
                            ...
                            pds[pdsSize] = null;
                            Result rc = interpreter.simpleUnify(t0, t1);
                            if (rc == FAIL) {
                                backtrackMode = true;
                            } else {
                                currentPosition++;
     ...
     *
     * @param env Environment to install the predicate
     */
    ...
    public String toString() {
        ...
        for (i = 0; i < n; i++) {
            rc.append(exceptionHandlers[i]).append("\n");
        }

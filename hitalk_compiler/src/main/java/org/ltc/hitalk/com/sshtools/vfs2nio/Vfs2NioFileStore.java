...
public
class Vfs2NioFileStore extends FileStore {
    private static
    class CVFS2NioFSFileStoreAttributes {
        Vfs2NioFileStore fstore;

        public
        CVFS2NioFSFileStoreAttributes ( Vfs2NioFileStore fstore ) throws IOException {
            this.fstore = fstore;
        }
        ...
        long unallocatedSpace () throws IOException {
            if (!fstore.isReadOnly()) return fstore.fs.getUnallocatedSpace();
            return 0;
        }

        public
        long usableSpace () throws IOException {
            if (!fstore.isReadOnly()) return fstore.fs.getUsableSpace();
            return 0;
        }
    }
    ...
    Object getAttribute ( String attribute ) throws IOException {
        if (attribute.equals("totalSpace")) return getTotalSpace();
        if (attribute.equals("usableSpace")) return getUsableSpace();
        if (attribute.equals("unallocatedSpace")) return getUnallocatedSpace();
